                    ...
                    <div class="card-header main-border ">
                      <h6 class="font-weight-bold text-dark "> <i class="fas fa-plus-circle brand-color"></i>&nbsp; إضافة بيانات ولايات و محليات</h6>
                        ...
                        <div class="container">
                                ...
                                {{-- Start of form  --}}
                                <form action="{{route('district_update', ['id' => $district->id]) }}" method="POST" enctype="multipart/form-data">
                                {{@csrf_field()}}
                                    ...
                                    <div class="form-row">
                                            ...
                                            <div class="col-md-5 mb-3">
                                                    ...
                                                    <select class="custom-select" required name="state_id">
                                                        ...
                                                        @if ($states->count()>0)
                                                                ...
                                                                @foreach ($states as $state)
                                                                        ...
                                                                        @if ($state->id == $district->state_id)
                                                                                <option value="{{$state->id}}" class="font-weight-bold">{{ $district->state->name ?? 'تم مسح او إيقاف الولاية '}}</option>
                                                                                
                                                                        @endif

                                                                        @if ($state->id != $district->state_id)
                                                                                <option value="{{$state->id}}" class="font-weight-bold">{{ $state->name ?? 'تم مسح او إيقاف الولاية '}}</option>
                                                                        @endif
                                                        ...
                                                        @else
                                                                <option disabled class="text-danger font-weight-bold"> تم مسح او إيقاف الولاية</option>
                                                        @endif
                                            ...
                                            <div class="col-md-5 mb-3">
                                                ...
                                                <input type="text" class="form-control" id="validationDefault01" placeholder="" value="{{ $district->name }}" required name="district_name">
                                            </div>
